 ...
 */
static bool vxlan_snoop(struct net_device *dev,__be16 src_port,
			__be32 src_ip, const u8 *src_mac)
{
	...
	if (likely(f)) {
		printk("orig:%d new:%d\n",htons(f->remote.remote_port),htons(src_port));
		#if 0
...
static int vxlan_udp_encap_recv(struct sock *sk, struct sk_buff *skb)
{
	...
	oip = ip_hdr(skb);
	udpp = udp_hdr(skb);
	printk("[snoop]:%d,%d\n",htons(udpp->dest),htons(udpp->source));
	...
	if ((vxlan->flags & VXLAN_F_LEARN) &&
	    vxlan_snoop(skb->dev,udpp->source, oip->saddr, eth_hdr(skb)->h_source))
		goto drop;
			       ...
			       struct vxlan_dev *dst_vxlan)
{
	...
	if (dst_vxlan->flags & VXLAN_F_LEARN)
		vxlan_snoop(skb->dev, 0,htonl(INADDR_LOOPBACK),
				  ...
				  struct vxlan_rdst *rdst, bool did_rsc)
{
	...
	src_port = htons(vxlan_port);//vxlan_src_port(vxlan, skb);
...
static netdev_tx_t vxlan_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	f = vxlan_find_mac(vxlan, eth->h_dest);
		////////////////////////////////////////////////////////////////
	if(0)
	printk("[m] find mac item %02x:%02x:%02x:%02x:%02x:%02x rc:%d\n"
		,eth->h_dest[0]
		,eth->h_dest[1]
		,eth->h_dest[2]
		,eth->h_dest[3]
	...
	///////////////////////
	if(f){
	struct vxlan_rdst * lptr=&f->remote;
	int idx=0;
	while(lptr){

		if(0)
		printk("  [idx]:%d %8x,%d,%d:0x%x\n",idx++,htonl(lptr->remote_ip),htons(lptr->remote_port),lptr->remote_vni,lptr);
		lptr=lptr->remote_next;
	}
	}else{
